...
const Finance = () => {
  ...
  const fetchFinances = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL + '/api' : 'http://localhost:5000/api'}/finances?businessId=${selectedBusiness.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL + '/api' : 'http://localhost:5000/api'}/finances/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL + '/api' : 'http://localhost:5000/api'}/finances`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      await fetchFinances();
