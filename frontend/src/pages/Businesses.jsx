...
const Businesses = () => {
  ...
  const handleDelete = async (e, id) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL + '/api' : 'http://localhost:5000/api'}/businesses/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL + '/api' : 'http://localhost:5000/api'}/businesses`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      await fetchBusinesses();
